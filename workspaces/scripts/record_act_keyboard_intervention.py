...
class Recorder:
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        self.segment_started_monotonic: Optional[float] = None
        self.pause_started_monotonic: Optional[float] = None
        self.pause_started_utc: Optional[str] = None
        self.current_segment_info: Optional[Dict[str, object]] = None
        self.active_segments: List[Dict[str, object]] = []
        self.pause_intervals: List[Dict[str, object]] = []
        self.rosbag_segment_ok_list: List[bool] = []
        self.switch_step_timeout_sec = 15.0
        ...
        self.policy_stop_cmd = str(args.policy_stop_cmd).strip()
        self.policy_start_cmd = str(args.policy_start_cmd).strip()
        self.policy_hook_timeout_sec = max(1.0, float(args.policy_hook_timeout_sec))
        self.policy_hook_events: List[Dict[str, object]] = []
        self.policy_launch_index = 0
        self.policy_proc: Optional[subprocess.Popen] = None
        self.policy_proc_log_handle = None
    ...
    def _run_switch_steps(self, action_name: str, steps: List[List[str]]) -> Tuple[bool, Dict[str, object]]:
        ...
        for idx, cmd in enumerate(steps, start=1):
            ...
            try:
                ...
                step_info["returncode"] = result.returncode
                step_info["stdout"] = self._trim_command_output(result.stdout)
                step_info["stderr"] = self._trim_command_output(result.stderr)
                step_ok = result.returncode == 0
            except subprocess.TimeoutExpired as exc:
                step_info["returncode"] = None
            ...
            step_ended_mono = time.monotonic()
            step_info["ended_at_utc"] = utc_now_iso()
            step_info["duration_sec"] = max(0.0, step_ended_mono - step_started_mono)
            ...
            if step_ok:
                self._print_colored(
                    f"[switch][{action_name}] step {idx}/{total_steps} OK",
                    color="green",
                )
                continue

            all_ok = False
            rc = step_info.get("returncode")
            self._print_colored(
                f"[switch][{action_name}] step {idx}/{total_steps} FAIL rc={rc}",
                color="red",
            )
            break
    ...
    def _close_policy_log_handle(self) -> None:
        if self.policy_proc_log_handle is None:
            return
        self.policy_proc_log_handle.flush()
        self.policy_proc_log_handle.close()
        self.policy_proc_log_handle = None
    ...
    def _start_policy_process(self, trigger: str) -> bool:
        ...
        if not self.policy_start_cmd:
            event["stderr"] = "empty policy start command"
            event["ended_at_utc"] = utc_now_iso()
            event["duration_sec"] = max(0.0, time.monotonic() - started_mono)
            self.policy_hook_events.append(event)
            return False

        if self.policy_proc is not None and self.policy_proc.poll() is None:
            event["stderr"] = "policy process already running"
            event["ended_at_utc"] = utc_now_iso()
            event["duration_sec"] = max(0.0, time.monotonic() - started_mono)
            self.policy_hook_events.append(event)
            return False
        ...
        if self.current_episode_dir is None:
            event["stderr"] = "episode directory is not set"
            event["ended_at_utc"] = utc_now_iso()
            event["duration_sec"] = max(0.0, time.monotonic() - started_mono)
            self.policy_hook_events.append(event)
            return False
        ...
        policy_log_path = self.current_episode_dir / f"policy_run_{self.policy_launch_index:03d}.log"
        event["log_path"] = str(policy_log_path)

        try:
            self.policy_proc_log_handle = policy_log_path.open("w", encoding="utf-8")
            self.debug_log_paths.append(policy_log_path)
            self.policy_proc = subprocess.Popen(
                self.policy_start_cmd,
                shell=True,
                executable="/bin/bash",
                stdout=self.policy_proc_log_handle,
            ...
            if early_rc is not None:
                event["returncode"] = early_rc
                event["stderr"] = f"policy exited immediately rc={early_rc}"
                ...
                self.policy_hook_events.append(event)
                self._print_colored(
                    f"[policy][start] FAIL trigger={trigger} rc={early_rc}",
                    color="red",
                )
                return False
        except Exception as exc:
            event["stderr"] = self._trim_command_output(str(exc))
            self.policy_proc = None
            self._close_policy_log_handle()
            event["ended_at_utc"] = utc_now_iso()
            event["duration_sec"] = max(0.0, time.monotonic() - started_mono)
            self.policy_hook_events.append(event)
            self._print_colored(f"[policy][start] FAIL trigger={trigger} err={exc}", color="red")
            return False
        ...
        self.policy_hook_events.append(event)
        self._print_colored(
            f"[policy][start] OK trigger={trigger} pid={self.policy_proc.pid if self.policy_proc else 'unknown'}",
            color="green",
        )
        return True
    ...
    def _stop_policy_process(self, trigger: str) -> bool:
        ...
        event: Dict[str, object] = {
            ...
            "stdout": "",
            "stderr": "",
            "managed_process_pid": self.policy_proc.pid if self.policy_proc is not None else None,
            "managed_process_stopped": True,
        }

        # Optional explicit stop hook first.
        if self.policy_stop_cmd:
            try:
                res = subprocess.run(
                    self.policy_stop_cmd,
                    shell=True,
                    executable="/bin/bash",
                    text=True,
                    capture_output=True,
                    timeout=self.policy_hook_timeout_sec,
                    check=False,
                )
                event["returncode"] = res.returncode
                event["stdout"] = self._trim_command_output(res.stdout)
                event["stderr"] = self._trim_command_output(res.stderr)
                if res.returncode != 0:
                    event["ok"] = False
            except subprocess.TimeoutExpired as exc:
                event["ok"] = False
                event["stderr"] = self._trim_command_output(
                    f"stop command timeout after {self.policy_hook_timeout_sec:.1f}s: {exc}"
                )
            except Exception as exc:
                event["ok"] = False
                event["stderr"] = self._trim_command_output(f"stop command exception: {exc}")

        managed_ok = True
        if self.policy_proc is not None:
            if self.policy_proc.poll() is None:
                managed_ok = self._stop_process_group(self.policy_proc, name="policy")
            if not managed_ok:
                event["managed_process_stopped"] = False
                event["ok"] = False
            self.policy_proc = None
        ...
        if bool(event.get("ok")):
            self._print_colored(f"[policy][stop] OK trigger={trigger}", color="green")
            return True

        self._print_colored(
            f"[policy][stop] FAIL trigger={trigger} rc={event.get('returncode')}",
            color="red",
        )
        return False
    ...
    def _active_elapsed_now(self, now: Optional[float] = None) -> float:
        elapsed = float(self.active_elapsed_sec)
        if self.recording and self.segment_started_monotonic is not None:
            ref = now if now is not None else time.monotonic()
            elapsed += max(0.0, ref - self.segment_started_monotonic)
    ...
    def _emit_runtime_status(self, force: bool = False) -> None:
        now = time.monotonic()
        if self.episode_open and (force or (now - self.last_record_status_ts) >= self.record_status_interval_sec):
            active_elapsed = self._active_elapsed_now(now=now)
            if self.recording:
                print(
                    f"[record] ACTIVE {self.current_episode_name} elapsed={self._format_duration(active_elapsed)} "
                    f"pending_saves={self._pending_save_count()}"
                )
            elif self.paused:
                paused_for = None
                if self.pause_started_monotonic is not None:
                    paused_for = max(0.0, now - self.pause_started_monotonic)
                print(
                    f"[record] PAUSED {self.current_episode_name} elapsed={self._format_duration(active_elapsed)} "
                    f"paused_for={self._format_duration(paused_for)} pending_saves={self._pending_save_count()}"
                )
    ...
    def _finalize_episode_job(self, job_id: int, job_data: Dict[str, object]) -> Dict[str, object]:
        ...
        metadata = {
            ...
            "timing": {
                "wall_duration_sec": job_data.get("wall_duration_sec"),
                "paused_duration_sec": job_data.get("paused_duration_sec"),
                "pause_count": job_data.get("pause_count"),
                "active_segments": list(job_data.get("active_segments", [])),
                "pause_intervals": list(job_data.get("pause_intervals", [])),
            },
            "control_switch": {
                "pause_steps": list(job_data.get("switch_pause_steps", [])),
                "resume_steps": list(job_data.get("switch_resume_steps", [])),
                "events": list(job_data.get("mode_switch_events", [])),
                "policy_start_cmd": job_data.get("policy_start_cmd", ""),
                "policy_stop_cmd": job_data.get("policy_stop_cmd", ""),
                "policy_events": list(job_data.get("policy_hook_events", [])),
            },
            ...
            "recording": {
                ...
                "rosbag_segment_ok_list": list(job_data.get("rosbag_segment_ok_list", [])),
            },
            "topics": {
            ...
            "rosbag_process_terminated_cleanly": bool(
                job_data.get("rosbag_ok_all", job_data.get("rosbag_ok", False))
            ),
            "save_job_id": job_id,
        }
    ...
    def _reset_current_episode_state(self) -> None:
        ...
        self.segment_index = 0
        self.active_elapsed_sec = 0.0
        self.segment_started_monotonic = None
        self.pause_started_monotonic = None
        self.pause_started_utc = None
        self.current_segment_info = None
        self.active_segments = []
        self.pause_intervals = []
        self.rosbag_segment_ok_list = []
        self.mode_switch_events = []
        self.policy_hook_events = []
        self.policy_launch_index = 0
        self.policy_proc = None
        self._close_policy_log_handle()
        self.current_topics = []
    ...
    def _segment_bag_prefix(self, episode_dir: Path, segment_index: int) -> Path:
        if segment_index <= 1:
            return episode_dir / "episode"
        return episode_dir / f"episode_seg{segment_index:03d}"

    def _segment_log_path(self, episode_dir: Path, segment_index: int) -> Path:
        if segment_index <= 1:
            return episode_dir / "rosbag_record.log"
        return episode_dir / f"rosbag_record_seg{segment_index:03d}.log"

    def _start_record_segment(self) -> bool:
        if not self.episode_open or self.paused or self.recording:
            return False
        if self.current_episode_dir is None:
            return False
        ...
        bag_prefix = self._segment_bag_prefix(self.current_episode_dir, segment_index)
        rosbag_log_path = self._segment_log_path(self.current_episode_dir, segment_index)
        ...
        try:
            ...
            self.rosbag_proc = None
            self.segment_index -= 1
            if self.debug_log_paths and self.debug_log_paths[-1] == rosbag_log_path:
                self.debug_log_paths.pop()
            raise

        segment_started_utc = utc_now_iso()
        segment_started_mono = time.monotonic()
        self.segment_started_monotonic = segment_started_mono
        self.recording = True
        segment_info: Dict[str, object] = {
            "segment_index": segment_index,
            "bag_prefix": str(bag_prefix),
            "rosbag_log": str(rosbag_log_path),
            "started_at_utc": segment_started_utc,
            "ended_at_utc": None,
            "duration_sec": None,
            "stop_reason": None,
            "rosbag_ok": None,
        }
        self.current_segment_info = segment_info
        self.active_segments.append(segment_info)
        print(f"[record] SEGMENT START idx={segment_index} bag_prefix={bag_prefix}")
        return True

    def _stop_record_segment(self, segment_stop_reason: str) -> bool:
        ...
        segment_ended_utc = utc_now_iso()
        segment_ended_mono = time.monotonic()
        segment_duration = None
        if self.segment_started_monotonic is not None:
            segment_duration = max(0.0, segment_ended_mono - self.segment_started_monotonic)
            self.active_elapsed_sec += segment_duration

        if self.current_segment_info is not None:
            self.current_segment_info["ended_at_utc"] = segment_ended_utc
            self.current_segment_info["duration_sec"] = segment_duration
            self.current_segment_info["stop_reason"] = segment_stop_reason
            self.current_segment_info["rosbag_ok"] = rosbag_ok
            seg_idx = self.current_segment_info.get("segment_index")
        else:
            seg_idx = self.segment_index
        self.rosbag_segment_ok_list.append(bool(rosbag_ok))
        self.current_segment_info = None
        self.segment_started_monotonic = None
        self.rosbag_proc = None
        self.recording = False
        ...
        print(
            f"[record] SEGMENT STOP idx={seg_idx} reason={segment_stop_reason} "
            f"duration={self._format_duration(segment_duration)} rosbag_ok={rosbag_ok}"
        )
        return rosbag_ok

    def _close_pause_interval(self, ended_at_utc: str, ended_at_monotonic: float) -> Optional[float]:
        if self.pause_started_monotonic is None:
            return None
        duration = max(0.0, ended_at_monotonic - self.pause_started_monotonic)
        pause_info = {
            "pause_index": len(self.pause_intervals) + 1,
            "started_at_utc": self.pause_started_utc,
            "ended_at_utc": ended_at_utc,
            "duration_sec": duration,
        }
        self.pause_intervals.append(pause_info)
        self.pause_started_monotonic = None
        self.pause_started_utc = None
        return duration
    ...
    def _watchdog_loop(self) -> None:
        while not self.shutdown_requested:
            ...
            if self.recording and self.rosbag_proc is not None and self.rosbag_proc.poll() is not None:
                self._set_pending_stop("process_error")
                continue

            if self.recording and self.policy_proc is not None and self.policy_proc.poll() is not None:
                self.current_warnings.append("Policy process exited unexpectedly while recording.")
                self._set_pending_stop("policy_process_exit")
    ...
    def _start_episode(self) -> None:
        ...
        if not switch_ok:
            step_entries = switch_event.get("steps", [])
            ...
            warning = (
                "Start align switch commands failed "
                f"(step={failed_text}); episode start aborted."
            )
            self.current_warnings.append(warning)
            self._print_colored(f"[record] WARN {warning}", color="red")
            self._stop_hz_monitors()
            self._reset_current_episode_state()
            return

        if not self._start_record_segment():
            self.current_warnings.append("Failed to start initial record segment.")
            self._stop_hz_monitors()
            self._reset_current_episode_state()
            return

        if not self._start_policy_process("episode_start"):
            self.current_warnings.append("Policy start failed on episode start.")
            self._stop_episode("policy_start_failed")
            return
        ...
        print(f"[record] bag_prefix={self._segment_bag_prefix(ep_dir, 1)}")
        print(f"[record] topics={len(self.current_topics)} optional={len(self.current_optional_recorded)}")

    def _pause_episode(self) -> None:
        if not self.episode_open or not self.recording or self.paused:
            return
        episode_name = self.current_episode_name or f"episode_{self.next_episode_index:03d}"

        if not self._stop_policy_process("pause"):
            self.current_warnings.append("Policy stop failed during pause.")
            self._stop_episode("policy_stop_failed")
            return

        self._stop_record_segment("user_pause")
        self.paused = True
        self.pause_started_utc = utc_now_iso()
        self.pause_started_monotonic = time.monotonic()
        active_elapsed = self._active_elapsed_now(now=self.pause_started_monotonic)
        self._print_colored(f"[record] PAUSE {episode_name}", color="green")
        print(f"[record] elapsed={self._format_duration(active_elapsed)}")
        ...
        if not switch_ok:
            ...
            failed_text = ",".join(failed_steps) if failed_steps else "unknown"
            warning = f"Pause switch commands failed (step={failed_text}); recorder remains paused."
            self.current_warnings.append(warning)
            self._print_colored(f"[record] WARN {warning}", color="red")
    ...
    def _resume_episode(self) -> None:
        ...
        if not switch_ok:
            ...
            failed_text = ",".join(failed_steps) if failed_steps else "unknown"
            warning = f"Resume switch commands failed (step={failed_text}); recorder stays paused."
            self.current_warnings.append(warning)
            self._print_colored(f"[record] WARN {warning}", color="red")
            return

        ok, preflight = self._wait_preflight(
            timeout_sec=self.args.restart_preflight_timeout_sec,
            skip_camera_publishers=True,
        )
        if not ok:
            reason = preflight.get("reason", "preflight_failed")
            print(f"[record] Resume preflight failed: {reason}")
            return

        pause_started_monotonic = self.pause_started_monotonic
        pause_started_utc = self.pause_started_utc
        self.paused = False
        if not self._start_record_segment():
            self.paused = True
            self.pause_started_monotonic = pause_started_monotonic
            self.pause_started_utc = pause_started_utc
            self.current_warnings.append("Failed to resume record segment.")
            return

        resumed_mono = self.segment_started_monotonic or time.monotonic()
        resumed_utc = (
            str(self.current_segment_info.get("started_at_utc"))
            if self.current_segment_info is not None
            else utc_now_iso()
        )
        pause_duration = self._close_pause_interval(resumed_utc, resumed_mono)

        if not self._start_policy_process("resume"):
            self.current_warnings.append("Policy start failed during resume.")
            self._stop_episode("policy_start_failed")
            return

        episode_name = self.current_episode_name or f"episode_{self.next_episode_index:03d}"
        self._print_colored(f"[record] RESUME {episode_name}", color="green")
        print(f"[record] paused_duration={self._format_duration(pause_duration)}")
    ...
    def _stop_episode(self, stop_reason: str) -> None:
        ...
        policy_stop_ok = self._stop_policy_process("episode_stop")
        if not policy_stop_ok:
            self.current_warnings.append("Policy stop failed during episode stop.")

        if self.recording:
            self._stop_record_segment(stop_reason)

        ended_monotonic = time.monotonic()
        ended_utc = utc_now_iso()
        if self.paused:
            self._close_pause_interval(ended_utc, ended_monotonic)
            self.paused = False

        hz_log_paths = self._stop_hz_monitors()

        active_duration_sec = self._active_elapsed_now(now=ended_monotonic)
        wall_duration_sec = None
        if self.current_started_monotonic is not None:
            wall_duration_sec = max(0.0, ended_monotonic - self.current_started_monotonic)
        paused_duration_from_intervals = sum(
            float(pause.get("duration_sec", 0.0)) for pause in self.pause_intervals
        )
        if wall_duration_sec is None:
            paused_duration_sec = paused_duration_from_intervals
        else:
            paused_duration_sec = max(0.0, wall_duration_sec - active_duration_sec)
        pause_count = len(self.pause_intervals)
        rosbag_ok_all = bool(self.rosbag_segment_ok_list) and all(self.rosbag_segment_ok_list)
        ...
        job_data: Dict[str, object] = {
            ...
            "active_segments": copy.deepcopy(self.active_segments),
            "pause_intervals": copy.deepcopy(self.pause_intervals),
            "switch_pause_steps": [self._cmd_to_string(step) for step in self.pause_switch_steps],
            "switch_resume_steps": [self._cmd_to_string(step) for step in self.resume_switch_steps],
            "mode_switch_events": copy.deepcopy(self.mode_switch_events),
            "policy_start_cmd": self.policy_start_cmd,
            "policy_stop_cmd": self.policy_stop_cmd,
            "policy_hook_events": copy.deepcopy(self.policy_hook_events),
            ...
            "rosbag_ok": rosbag_ok_all,
            "rosbag_ok_all": rosbag_ok_all,
            "rosbag_segment_ok_list": list(self.rosbag_segment_ok_list),
        }

        save_job_id = self._submit_save_job(job_data)

        self.last_stop_monotonic = ended_monotonic
        ...
        print(f"[record] duration={self._format_duration(active_duration_sec)} (active)")
        print(f"[record] wall_duration={self._format_duration(wall_duration_sec)}")
        print(f"[record] paused_duration={self._format_duration(paused_duration_sec)} pause_count={pause_count}")
    ...
    def _atexit_cleanup(self) -> None:
        ...
        elif self.policy_proc is not None and self.policy_proc.poll() is None:
            self._stop_policy_process("atexit")
        ...
        try:
            with RawTerminal():
                while not self.shutdown_requested:
                    ...
                    if pending and self.episode_open:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            "Shell command to launch policy process. "
            "Starts after recording begins and on resume."
        ),
    )
    parser.add_argument(
        "--policy-stop-cmd",
        default="",
        help=(
            "Optional shell command to stop policy process before pause/stop. "
            "If empty, recorder stops managed policy process directly."
        ),
    )
    parser.add_argument(
        "--policy-hook-timeout-sec",
        type=float,
        default=15.0,
        help="Timeout for policy stop/start hook commands.",
    )
