...
source_ros_setup() {
  ...
  echo "[build_teleop_ws] ROS setup not found under /opt/ros/{noetic,melodic}." >&2
  return 1
}
...
if command -v catkin >/dev/null 2>&1; then
  ...
  echo "[build_teleop_ws] Neither catkin nor catkin_make is available in PATH." >&2
  exit 1
fi

echo "[build_teleop_ws] Build complete: ${WS_DIR}"
