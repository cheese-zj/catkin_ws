#!/usr/bin/env python3
"""Publish 3-arm manual command sequence.
...
class ThreeArmZeroNode:
    def __init__(self, args: argparse.Namespace):
        ...
        self.opp_pos: Optional[np.ndarray] = None
        self.left_stamp: Optional[float] = None
        self.right_stamp: Optional[float] = None
        self.opp_stamp: Optional[float] = None
        ...
        self.pub_opp = rospy.Publisher(args.out_opp_topic, JointState, queue_size=1)
        ...
        self.subscribers = [
            ...
            rospy.Subscriber(
                args.robot_opp_topic, JointState, self._cb_opp_joint, queue_size=1, tcp_nodelay=True
            ),
        ]
    ...
    def _cb_opp_joint(self, msg: JointState):
        if len(msg.position) < PER_ARM_DIM:
            rospy.logwarn_throttle(
                2.0, "Opp state short dims. Need >=%d position, got %d", PER_ARM_DIM, len(msg.position)
            )
            return
        with self.lock:
            self.opp_pos = np.asarray(msg.position[:PER_ARM_DIM], dtype=np.float32)
            self.opp_stamp = self._stamp_to_sec(msg.header.stamp)

    def _snapshot(self) -> Optional[Tuple[np.ndarray, np.ndarray, np.ndarray, Tuple[float, float, float]]]:
        with self.lock:
            if self.left_pos is None or self.right_pos is None or self.opp_pos is None:
                return None
            return (
                self.left_pos.copy(),
                self.right_pos.copy(),
                self.opp_pos.copy(),
                (float(self.left_stamp), float(self.right_stamp), float(self.opp_stamp)),
            )
    ...
    def _publish_triple(self, left: np.ndarray, right: np.ndarray, opp: np.ndarray):
        stamp = rospy.Time.now()
        self._publish_single(self.pub_left, left, stamp)
        self._publish_single(self.pub_right, right, stamp)
        self._publish_single(self.pub_opp, opp, stamp)
    ...
    def _run_phase(
        ...
        while not rospy.is_shutdown():
            ...
            opp_cmd = self._lerp(opp_start, opp_end, alpha)
            self._publish_triple(left_cmd, right_cmd, opp_cmd)
            if alpha >= 1.0:
                break
            rate.sleep()
        # Publish final once more to make endpoint explicit.
        self._publish_triple(left_end, right_end, opp_end)
    ...
    def run(self):
        ...
        rospy.loginfo("run_gripper_open_then_zero_ros_3arm started")
        rospy.loginfo(
            "Output topics: left=%s right=%s opp=%s",
            self.args.out_left_topic,
            self.args.out_right_topic,
            self.args.out_opp_topic,
        )

        while not rospy.is_shutdown():
            snap = self._snapshot()
            if snap is None:
                rospy.loginfo_throttle(2.0, "Waiting for 3-arm state streams...")
                rate.sleep()
                continue
            left_start, right_start, opp_start, stamps = snap
            if self.args.debug_streams:
                rospy.loginfo(
                    "Initial stamps(left,right,opp)=(%.3f,%.3f,%.3f)",
        ...
        opp_zero = np.zeros(PER_ARM_DIM, dtype=np.float32)
        self._run_phase(
            left_open,
            right_open,
            opp_open,
        ...
        while not rospy.is_shutdown():
            self._publish_triple(left_zero, right_zero, opp_zero)
            rate.sleep()
...
def make_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--robot-opp-topic", default="/robot/arm_opp/joint_states_single")
    ...
    parser.add_argument("--out-opp-topic", default="/robot/arm_opp/vla_joint_cmd")
    return parser
...
def main():
    ...
    rospy.init_node("run_gripper_open_then_zero_ros_3arm")
    node = ThreeArmZeroNode(args)
    node.run()
